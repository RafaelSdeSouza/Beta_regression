...
b_all <- rbind(b1,b2,b3,b4,b5)

b_all$case <- rep(c("a = 0.5 b = 0.5", "a = 5 b = 1","a = 1 b = 3","a = 2 b = 2","a = 2 b = 5"),each=100)

pdf("beta_dist.pdf",width = 10,height = 8)
ggplot(b_all,aes(x=x,y=y,group=case,color=case,linetype=case)) +
  geom_line(name="",size=1.25) + scale_color_economist(name="") +
  scale_linetype_stata(name="") +
  theme_economist_white() +
  theme(legend.text=element_text(size=12.5),legend.background = element_rect(fill="transparent"),
